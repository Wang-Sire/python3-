...
# 发送邮件
#设置服务器所需信息
def SendMailInfomation(title, conent, fomrMail, password, toMail):
        #163邮箱服务器地址
...
class hackTickets(object):
    ...
    def login(self):
        ...
        # 自动填充用户名
        #self.driver.fill("loginUserDTO.J_userName", self.username)
        print("自动填充密码")
        # self.driver.fill("userDTO.password", self.passwd)

        print(u"等待验证码，自行输入...",self.driver.url, "要等于的url", self.initmy_url)

        # 验证码需要自行输入，程序自旋等待，直到验证码通过，点击登录
        while True:
            if self.driver.url != self.initmy_url and  "index"  not in str(self.driver.url) :
                print("Once  Url ： ",self.driver.url)
                sleep(1)
            else:
                break
    ...
    def searchMore(self):
        # 选择车次类型
        try:
            self.driver.find_by_text("确认").last.click()
            print("有 searchMore 确认弹框！")

        except:
            print("没有 searchMore 确认弹框！")
        ...
        for type in self.train_types:
            ...
            if type == 'T' or type == 'G' or type == 'D' or type == 'Z' or type == 'K':
                ...
                # sleep(5)
                # s = driver.find_by_id("qd_closeDefaultWarningWindowDialog_id")
                #print(s)
                # if len(s) == 1：
    ...
    def buyOrderZero(self):
        ...
        while self.driver.url == self.ticket_url:
            ...
            sleep(0.05)
            print("buyOrderZero 点击查询！ ")
            self.driver.find_by_text(u"查询").click()
            count += 1
            print(u" buyOrderZero循环点击查询... 第 %s 次" % count)
    ...
    def selUser(self):
        ...
        for user in self.users:
            ...
            try:
                print("有selUser 取消!")
                self.driver.find_by_text("取消").last.click()
            except:
                print("没有selUser 取消")
                continue
    ...
    def confirmSeat(self):
        ...
        if self.driver.find_by_text(u"硬座余票<strong>0</strong>张") == None:
            ...
            if self.noseat_allow == 0:
                ...
                self.driver.find_by_id('qr_submit_id').click()
        print("购票完成请尽快付款！")
        SendMailInfomation("OK","Please Quick Your Email Thank you!", "MailAccount", "MailPassword", tomail)
    ...
    def start(self):
        ...
        print(" 驱动加载完成！")
        # 登录，自动填充用户名、密码，自旋等待输入验证码，输入完验  证码，点登录后，访问 tick_url（余票查询页面）
        self.login()
